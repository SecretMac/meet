...
if not playerGui then warn("PlayerGui not found, script may fail.") return end
local packets = ReplicatedStorage and ReplicatedStorage:WaitForChild("LocalModules") and require(ReplicatedStorage:WaitForChild("LocalModules"):WaitForChild("Backend"):WaitForChild("Packets"))
...
local function createGUI()
    local ScreenGui = playerGui:FindFirstChild("TrollGui")
    ...
    if not MainFrame then
        ...
        MainFrame.Size = UDim2.new(0, 250, 0, 300) -- Increased height for timer
        ...
        TimerLabel.Text = "Time Left: 120s"
        ...
        ToggleButton.Position = UDim2.new(0.25, -50, 0, 110)
        ...
        TeleportButton.Position = UDim2.new(0.75, -50, 0, 110)
        ...
        ChatButton.Position = UDim2.new(0.5, -50, 0, 160)
        ...
        MessageButton.Position = UDim2.new(0.5, -50, 0, 210)
        ...
        InputFrame.Position = UDim2.new(0.5, -100, 0, 250)
    ...
    return MainFrame, ScreenGui, PlayerCountLabel, NotificationContainer, TimerLabel, ToggleButton, TeleportButton, ChatButton, MessageButton, InputFrame, InputBox, SendButton
end
...
local function updateCurrentPlayers(PlayerCountLabel)
    currentPlayers = {}
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= player then
...
Players.PlayerAdded:Connect(function(plr)
    updateCurrentPlayers(PlayerCountLabel)
...
local function serverHop()
    ...
    if req and TeleportService then
        local site = req({Url = "https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"})
        if site and site.Success then
            local body = game:GetService("HttpService"):JSONDecode(site.Body)
        ...
        if #servers > 0 then
            ...
            if NotificationContainer then
                createNotification("No available servers found.", Color3.fromRGB(255, 100, 100))
            end
        end
    else
        if NotificationContainer then
            createNotification("HTTP requests or TeleportService not supported.", Color3.fromRGB(255, 100, 100))
        end
    end
end

-- Main Function to Initialize Script
local scriptUrl = "https://raw.githubusercontent.com/0riginalWarrior/Stalkie/refs/heads/main/roblox.lua"
local queueTeleport = (syn and syn.queue_on_teleport) or (fluxus and fluxus.queue_on_teleport) or queue_on_teleport or function() end
local function initializeScript()
    local success, result = pcall(function()
        local MainFrame, ScreenGui, PlayerCountLabel, NotificationContainer, TimerLabel, ToggleButton, TeleportButton, ChatButton, MessageButton, InputFrame, InputBox, SendButton = createGUI()
        ...
        task.spawn(function()
            ...
            if NotificationContainer then
                createNotification("Server hopping in 5 seconds...", Color3.fromRGB(255, 165, 0))
            end
            task.wait(5)
            serverHop()
        end)
...
TeleportService.LocalPlayerArrivedFromTeleport:Connect(function(player, pastInfo)
    if player == player then
        task.wait(2)
        queueTeleport('loadstring(game:HttpGet("' .. scriptUrl .. '"))()')
        initializeScript()
...
-- Initial Execution
if not _G.StalkieScriptExecuted then
